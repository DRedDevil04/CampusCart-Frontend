    ...
    <Box p={4} maxW="800px" mx="auto">
      ...
      {updateError && !emailExistsError && (
        <Alert status="error" mt={4}>
          <AlertIcon />
          Error updating profile: {updateError.message} Possibly Email Already
          Exists with us.
        </Alert>
      )}
      ...
      <Box p={6} mb={6} bg="white" borderRadius="md" boxShadow="md">
        ...
        <Flex align="center">
          <Input
            ...
            readOnly={!enrollment_numberedit}
            mr={2}
          />
          {!enrollment_numberedit ? (
            <Button
              onClick={() => setEnrollment_NumberEdit(true)}
              colorScheme="blue"
            >
              Edit
            </Button>
            ...
            <Button
              onClick={() => handleProfileUpdate("enrollment_number")}
              isLoading={isUpdating}
              loadingText="Saving..."
              colorScheme="green"
            >
      ...
      <Stack spacing={6}>
        {userResponse.data.orders.length === 0 ? (
          <Text mb={2}>No Order Found!</Text>
        ) : null}
        {userResponse.data.orders &&
          userResponse.data.orders.map((order, index) => (
            <Box key={index} p={6} bg="white" borderRadius="md" boxShadow="md">
              <Text mb={2}>
                <strong>Order Amount:</strong> {order.payment.amount}{" "}
                {order.payment.currency}
              </Text>
              <Text mb={2}>
                <strong>Order Placed:</strong>{" "}
                {new Date(order.__created).toLocaleDateString()}
              </Text>
              <Text mb={2}>
                <strong>Ship To:</strong> Room No. -{" "}
                {order.shipping.address.room} <br />
                {order.shipping.address.floor} floor,{" "}
                {order.shipping.address.hostel},{" "}
                {order.shipping.address.contact_number}
              </Text>
              <Text mb={2}>
                <strong>Mode of Payment:</strong> {order.payment.payment_method}
              </Text>
              <Box
                p={2}
                borderRadius="md"
                bg={getStatusColor(order.payment.status)}
                textAlign="center"
                mb={4}
              >
